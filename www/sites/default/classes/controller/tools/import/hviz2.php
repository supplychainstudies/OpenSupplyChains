...
class Controller_Tools_Import_Hviz2 extends Controller_Tools_Import_Xls {
    
    public $layout = 'base';
    public $template = 'tools/import/hviz';
    ...
    public function action_index() {
        ...
        if(strtolower(Request::$method) === 'post') {
            ...
            if(isset($posted->hviz_file) && $posted->hviz_file instanceof Sourcemap_Upload && $posted->hviz_file->ok()) {
                $xls = $posted->hviz_file->get_contents();
                try {
                    $sc = Sourcemap_Import_Hviz2::hviz2sc($xls, $posted);
                } catch(Exception $e) {
                    die($e);
                    Message::instance()->set('Problem with import: '.$e->getMessage());
                    $this->request->redirect('tools/import/hviz');
                }
