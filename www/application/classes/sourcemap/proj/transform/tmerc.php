<?php
class Sourcemap_Proj_Transform_Tmerc extends Sourcemap_Proj_Transform {
/*******************************************************************************
...
*******************************************************************************/
    ...
    public function forward($p) {
        ...
        $delta_lon = Sourcemap_Proj::adjust_lon($lon - $this->long0); // Delta longitude
        # $con;    // cone constant
        $sin_phi = sin($lat);
        $cos_phi = cos($lat);
    ...
    public function inverse($p) {
        ...
        if($this->sphere) {   /* spherical form */
            ...
            for ($i=0; true; $i++) {
                ...
                $phi += $delta_phi;
                if(abs($delta_phi) <= Sourcemap_Proj::EPSLN) break;
                if($i >= $max_iter) {
