...
<?php else: ?>
<option <?php if(isset($selected, $tree->data->id) && $selected == $tree->data->id): ?>selected <?php endif; ?>value="<?php HTML::chars(isset($tree->data->id) ? $tree->data->id : -1) ?>"><?php str_pad('', $indent, '-') ?><?php HTML::chars($tree->data->title) ?></option>
<?php if($tree->children): ?>
<?php foreach($tree->children as $i => $child): ?>
<?php View::factory('partial/admin/taxonomy/options', array(
