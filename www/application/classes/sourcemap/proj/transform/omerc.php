<?php
class Sourcemap_Proj_Transform_Omerc extends Sourcemap_Proj_Transform {
/*******************************************************************************
...
*******************************************************************************/
    ...
    public function init() {
        ...
        if ($this->mode != 0) {
            ...
            if($this->dlon < -Sourcemap_Proj::PI) $this->lon2 = $this->lon2 - 2.0 * Sourcemap_Proj::PI;
            if($this->dlon > Sourcemap_Proj::PI) $this->lon2 = $this->lon2 + 2.0 * Sourcemap_Proj::PI;
            $this->dlon = $this->lon1 - $this->lon2;
...
#        $ts1;
        ...
        if (abs(abs($lat) - Sourcemap_Proj::HALF_PI) > Sourcemap_Proj::EPSLN) {
            ...
            if(abs($con) < .0000001) {
                ...
                if ($con < 0) $us = $us + Sourcemap_Proj::PI * $this->al / $this->bl;
            }
        } else {
            if($lat >= 0) {
                $ul = $this->singam;
