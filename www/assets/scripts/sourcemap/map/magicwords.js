...
Sourcemap.MagicWords.full_content = {
    "youtube": {
        "link": function(lnk) {
            if(!lnk || !lnk.match(/((\?v=)|(v\/))(.+)$/))
                return '<p class="error">Invalid YouTube link.</p>';
            ...
            if(m) {
                var mkup = '<iframe width="500" height="360" class="youtube-player" type="text/html"'+
                    'src="http://www.youtube.com/embed/'+m[4]+'?autoplay=0"'+ 
                    'frameborder="0" allowfullscreen></iframe>';
            } else mkup = '';
            return mkup;
        }
    ...
    "vimeo": {
        "link": function(lnk) {
            var mkup = '<iframe width="500" height="360" class="vimeo-player" src="http://player.vimeo.com/video/'+
                (lnk.match(/\/(\d+)$/))[1]+'?title=0&amp;byline=0&amp;portrait=0&autoplay=0" '+
                'frameborder="0"></iframe>';
            return mkup;
        }
    },
